...
Abra `http://localhost:8080` no navegado. 
...
## Atualização Recente (20/11)

Nesta última atualização, o projeto recebeu diversos aprimoramentos voltados à eficiência, confiabilidade e usabilidade.

### Melhorias no Cliente LoRa (sensor)

- Implementado deep sleep para reduzir o consumo de energia.

- Adicionada lógica de envio sob demanda, evitando transmissões redundantes.

- Inclusão do contador de sequência (seq) para análise de perda de pacotes.

- Ajustado o formato das mensagens LoRa para maior consistência.

### Melhorias no Gateway

- Filtro para evitar reenviar medições idênticas em intervalos curtos.

- Melhor detecção e tratamento de mensagens inválidas.

### Melhorias na Dashboard

- Implementada autoatualização automática (com opção de pausa).

- Cards passam a exibir alertas visuais para valores fora do ideal.

- Gráfico recebeu eixo Y com escala fixa por métrica (temperatura, umidade, poeira).

- Melhorias visuais e organização dos componentes.

- Nova coluna para visualização do número de sequência para rastreamento de perdas.

### Suporte a Testes Sem Hardware

- Script de simulação atualizado para gerar dados realistas, incluindo seq, alertas e histerese.

- Permite testar todo o sistema (sensor → gateway → servidor → dashboard) sem dispositivos físicos.
